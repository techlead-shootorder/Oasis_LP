...
const LEAD_URL_PREFIX =
  AppConstant.environment == "qa" ? QA_LEAD_URL_PREFIX : LIVE_LEAD_URL_PREFIX;
const SF_CLIENT_ID =
  AppConstant.environment == "qa" ? QA_SF_CLIENT_ID : LIVE_SF_CLIENT_ID;
const SF_CLIENT_SECRET =
  AppConstant.environment == "qa" ? QA_SF_CLIENT_SECRET : LIVE_SF_CLIENT_SECRET;
const SF_USERNAME =
  AppConstant.environment == "qa" ? QA_SF_USERNAME : LIVE_SF_USERNAME;
const SF_PASSWORD =
  AppConstant.environment == "qa" ? QA_SF_PASSWORD : LIVE_SF_PASSWORD;
...
export async function POST(request) {
  ...
  return NextResponse.json(
    {
      message:
        "Thank you for reaching out to us. Our execuitve will get back to you shortly.",
      response: response,
...
async function sendSalesforceLeadRequest(requestJson) {
  try {
    ...
    const authResponse = await fetch(LEAD_URL_PREFIX + "/services/oauth2/token", {
      method: "POST",
      headers: myHeaders,
      body: formdata,
      redirect: "follow",
    ...
    if (authData && authData.access_token) {
      ...
      const raw = JSON.stringify({
        ...
        Company:
          requestJson.message && requestJson.message.length > 0
            ? requestJson.message
            : "Oasis",
        ...
        Appointment_Date__c:
          requestJson.appointmentDate ?? formatDateToYYYYMMDD(new Date()),
      ...
      const response = await fetch(
        ...
        `/services/data/v60.0/sobjects/Lead/Phone__c/${encodeURIComponent(requestJson.mobileNo)}`,
        {
          method: "PATCH",
          headers: myHeaders,
          body: raw,
      ...
      const leadSubmitData = await response.json();

      console.log(
        "Salesforce Lead form submission response is ",
        leadSubmitData
      );
