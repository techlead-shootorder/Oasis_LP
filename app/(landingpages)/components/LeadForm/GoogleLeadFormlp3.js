...
const GoogleLeadFormlp3 = () => {
    ...
    const [isFocusedLookingFor, setIsFocusedLookingFor] = useState(false);
    ...
    const handleSendOtp = () => {
        const { firstName, mobileNo, gender, age, looking_for, consent } = userDetails;
        ...
        if (!looking_for) { // NEW VALIDATION
            setErrorMessage("Please select what you're looking for");
            return;
        }
    ...
    const handleSubmit = (e) => {
        ...
        if (!isFormValid()) {
            ...
            if (!userDetails.age) {
             ...
             if (!userDetails.looking_for) { // NEW VALIDATION
            setErrorMessage("Please select what you're looking for");
            return;
        }
    ...
    return (
        <>
            <div className="rounded-[27px] bg-cover bg-center bg-[#f3c1d7] overflow-hidden relative">
                <p className="text-white mb-4 p bg-primary text-center py-2 text-[18px]  sm:text-[22px] font-bold">Free Fertility Consultation</p>
    ...
    <select
        ...
        onChange={handleInputChange}
        onFocus={() => setIsFocusedLookingFor(true)}
        onBlur={() => setIsFocusedLookingFor(false)}
    ...
    {!isFocusedLookingFor && !userDetails.looking_for && (
        <span className="absolute left-[98px] top-2 text-red-500 text-2xl">*</span>
    )}
</div>
...
export default GoogleLeadFormlp3;